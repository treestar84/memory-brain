...
Show the user the command's output, including the created job path(s), and explain that drafts still need review/acceptance (`bun run "$CLAUDE_PLUGIN_ROOT/bin/cfgm.ts" capture-status`, `bun run "$CLAUDE_PLUGIN_ROOT/bin/cfgm.ts" capture-accept`) before they become part of the memory wiki.
