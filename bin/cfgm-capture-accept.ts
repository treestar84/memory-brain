...
function parseArgs(argv: string[]): { slug: string | null; type: string | null; force: boolean; json: boolean } {
...
const { slug, type, force, json } = parseArgs(process.argv.slice(2));

if (!slug || !type) {
  console.error("사용법: cfgm capture-accept <slug> --type concept|decision|project [--force]");
  process.exit(2);
}
...
const draftPath = resolve(repoRoot, "memory/_pending/capture/drafts", `${slug}.md`);
...
if (json) {
  console.log(JSON.stringify({ slug, type, targetPath, supersededExisting: targetExists }, null, 2));
} else {
