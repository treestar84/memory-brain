...
- `memory/_pending/normalize/_spec/ssl-schema.md` — SSL 0.3.0 schema 구조 명세 (4 신규 노드 포함)
- `memory/_pending/normalize/_spec/vocabulary.yaml` — closed vocabulary 단일 source. enum 검증 시 본 파일 우선 참조 (default + extensions)
- `memory/concepts/_ssl/_canonical/actions.yaml` — canonical action store. logical.actionRef 가 본 파일의 ID 참조. 매치되는 패턴은 반드시 ref 사용 (inline 금지 — 결정성 보장).
